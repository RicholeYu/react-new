...
export function isEqualOrHigherThanVersion (_version) {
    ...
    let version = (cookies.version || params.version || "1.0.0")
    let isEqual = true
    if (_version === version) {
        return true
    }
    ...
    for (let index = 0; index < version.length; index++) {
        if (isEqual && Number(version[index]) > Number(_version[index])) {
            return true
        } else if (Number(version[index]) < Number(_version[index])) {
            return false
        }
    }
    return false
}
window.isEqualOrHigherThanVersion = isEqualOrHigherThanVersion
    ...
    let eventPage = function () {
        try {
            window.gtag("event", eventName, {
                ...
                "event_label": uid ? `${eventLabel}_${uid}` : eventLabel,
                "value": 1
            })
        } catch (e) {
            setTimeout(() => {
...
export function getTime (time) {
    time = Number(time) > 9999999999 ? Number(time) : Number(time) * 1000
    let date = new Date()
    date.setTime(time)
    return [
        date.getFullYear(),
...
export function getURLParamsString (obj = {}) {
    let keys = []
    for (let name in obj) {
        keys.push(`${name}=${obj[name]}`)
    }
    return `?${keys.join("&")}`
}
...
export const cookie = {
    ...
    set (name, value, expired) {
        // 防止线上或线下配置ck
        if ((isProduction || isRelease) && (name === "ck" || name === "appck")) {
            return
        }
        if (expired === "always") {
            expired = 99999
...
export const faceBookReadyPromise = new Promise(resolve => {
    if (window.facebookReady) {
        resolve(true)
    } else {
...
export const faceBookLoginPromise = new Promise(resolve => {
    faceBookReadyPromise.then(() => {
        window.FB.getLoginStatus((response) => {
            console.log("verifyed", response.status)
            if (response.status) {
...
export function getLink (data = {}) {
    let search = getURLParamsString({
        ...data
    })
    return isProduction ?
        `https://www.madcrickets.com/api/tool/share/downpage${search}` :
        `https://prewww.madcrickets.com/api/tool/share/downpage${search}`
}

// 只有大于等1.2.9的包才可以用
export function openGooglePlay () {
    if (isEqualOrHigherThanVersion("1.2.9")) {
        cbetLocal({
            func: "openBrowser",
            params: {
                content: "https://play.google.com/store/apps/details?id=com.crazy500.cbet"
            }
        })
        return true
    }
